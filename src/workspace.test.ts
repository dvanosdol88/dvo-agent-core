...
import {
  ...
  buildWorkspaceSaveReceipt,
  getWorkspaceToolContract,
...
test("workspace contracts expose the canonical tool names in stable order", () => {
  assert.deepEqual(
    WORKSPACE_TOOL_CONTRACTS.map((contract) => contract.name),
    [
...
test("read_workspace_file_text contract requires a fileId", () => {
  ...
  assert.equal(
    contract?.inputSchema.properties?.fileId instanceof Object,
    true,
  );
...
test("workspace result types carry provider, source, retrieval, and metadata", () => {
  ...
  const readResult: WorkspaceFileReadResult = {
