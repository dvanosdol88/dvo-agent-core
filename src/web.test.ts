...
import {
  PUBLIC_WEB_TOOL_CONTRACTS,
  getPublicWebToolContract,
  type WebReadResult,
  type WebSourceRef,
} from "./index.js";
...
test("public web contracts expose the canonical tool names in stable order", () => {
  assert.deepEqual(
    PUBLIC_WEB_TOOL_CONTRACTS.map((contract) => contract.name),
    [
      "search_web",
      "read_web_url",
      "list_web_page_links",
...
test("read_web_url contract accepts a URL plus bounded extraction controls", () => {
  ...
  assert.equal(
    contract?.inputSchema.properties?.maxChars instanceof Object,
    true,
  );
  assert.equal(
    contract?.inputSchema.properties?.extractDepth instanceof Object,
    true,
  );
...
test("crawl_web_site contract requires instructions and defaults to bounded discovery", () => {
  ...
  assert.equal(contract?.inputSchema.properties?.url instanceof Object, true);
  assert.equal(
    contract?.inputSchema.properties?.instructions instanceof Object,
    true,
  );
  assert.equal(
    contract?.inputSchema.properties?.allowExternal instanceof Object,
    true,
  );
